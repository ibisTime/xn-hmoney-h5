...
import {addAdvertising, getBbListData, getAdvertisePrice, getAdvertiseDetail, ExitAdvertising, getAdverMessage, getAdvertiseBbList} from 'api/otc';
...
export default {
  ...
  mounted() {
    ...
    // this.config.tradeCurrency = sessionStorage.getItem('WALLET_CURRY') || 'CNY';
    ...
    getAdvertiseBbList().then(data => {
      ...
      this.getBbPrice(this.config.tradeCoin, this.config.tradeCurrency);
      this.getAdverDetail();
    });
  },
  methods: {
    ...
    getUserWallet(){
      wallet().then(data => {
        if(data.accountList.length > 0) {
          let wallet = data.accountList.filter(item => {
